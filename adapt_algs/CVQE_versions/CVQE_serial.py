...
class Classical_VQE(VQE):
	...
	def output_dressed_ham(self):
		...
		for term in self.ham_term_list:
			for descendant in term['descendants']:
				...
				term_sums[int(base_4_map(name[0]))] = np.real(term_sums[int(base_4_map(name[0]))]) + np.real(weight[0]*self.constants[i])
				other_sum_term = other_sum_term + np.real(weight[0]*self.constants[i])*pauli[0]
				i = i+1 
	...
	def run(self, return_lists = False, check_params = True):
		"""
		...
		if check_params:
			energy = self._evaluate_multi_energy(self.param_list)
			ret = {'min_val': energy, 'opt_params': self.param_list}
		else:
