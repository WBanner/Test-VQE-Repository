...
def retrieve_ham(number, num_qubits = 4):
    ...
    for counter2 in range(0, len(single_ham_list)-1,1):
        ...
        weight_list[counter2] = complex(single_ham_list[counter2][num_qubits+2:-1])
...
def retrieve_op_list(number, rev = False, num_qubits = 4):
    ...
    print('dict', adapt_op_dict['Ham_{}'.format(number)])
    print('label list', op_label_list)
    op_list = []
...
for i in range(0,1):
    ...
    while k < runs:
        ...
        #cvqe2 = CVQE(operator = qubitOp, optimizer = optimizer, operator_list = op_list, var_form = var_form, expec_mode = False, dir_to_bracket = False)
        #start = time.time()
        #cvqe_result = cvqe.run()
        #runtime = time.time()-start
        #start = time.time()
        #cvqe2_result = cvqe2.run()
