...
class OperatorPool:
    ...
    def thanos_pool(self, frac=1 / 2):
        ...
        """
        return np.random.choice(self.pool, int(frac * len(self.pool)))
...
def x_mixer(n: int):
    ...
    for o in PauliPool.from_pauli_strings(x_strings(n)).pool:
        op += o
    return op
...
def make_complete_pool(n_qubits: int) -> List[str]:
...
class ChemPool(PauliPool):
    def __init__(self, name=''):
        super().__init__(name=name)

    @classmethod
    def from_chem_operator_pool(cls, op_pool):
        try:
            import OperatorPoolOld
            if not isinstance(op_pool, OperatorPoolOld):
                raise ValueError('Input must be an instance of OperatorPool as defined in of-adapt-vqe')
            new_pool_list = op_pool.fermi_ops

            return cls.from_openfermion_qubit_operator_list(new_pool_list)
        except ImportError:
            raise ValueError('Cannot import legacy pool class')


class FermionicPool(OperatorPool):
    def __init__(self, name=''):
        super().__init__(name=name)

    @classmethod
    def from_fermionic_operator(cls, ferm_op, map_type):
        result = cls()
        ops_one_body, ops_two_body = fermionic_mapping_to_qubit_ops(
            ferm_op, map_type)
        result.add_ops(ops_one_body + ops_two_body)
        return result

    @classmethod
    def from_one_body_ops(cls, ferm_op, map_type):
        result = cls()
        ops_one_body, ops_two_body = fermionic_mapping_to_qubit_ops(
            ferm_op, map_type)
        result.add_ops(ops_one_body)
        return result

    @classmethod
    def from_two_body_ops(cls, ferm_op, map_type):
        result = cls()
        ops_one_body, ops_two_body = fermionic_mapping_to_qubit_ops(
            ferm_op, map_type)
        result.add_ops(ops_two_body)
        return result